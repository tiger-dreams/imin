...
| 기능 | 설명 |
|---|---|
| 행사 생성 | 모바일 청첩장처럼 제목, 행사 내용, 이미지, 일정, 장소, 정원, 승인 방식 설정 |
| 행사 홈/상세 | 공개 행사 목록, 내가 만든 행사, eventId 기반 초대장 상세 |
| 데모 행사 체크인 | 매일 오늘 날짜로 갱신되는 샘플 행사에서 참여 신청 후 현장 체크인 시작 |
| 통합 체크인 UX | 행사 상세에서 넘어온 체크인 화면에 행사명/장소/복귀 CTA를 표시하고 행사 플랫폼 테마 유지 |
| 참여 신청 | 공개 행사 신청, 주최자 승인, 대기/거절 상태 관리 |
| 접수 운영 | 접수 한도, 접수 시작/마감, 초과 확정률, 호스트/관리자 전용 신청자 관리 |
...
| 릴리즈 내역 | `/release`, `/release-notes` — 최근 GSD 작업, 검증 결과, 관련 이슈와 QA 로그 확인 |
| 블로그 | `/blog` — imin 활용법과 제품 관점 아티클 |
...
│       ├── ReleaseNotesPage.tsx # 공개 릴리즈 노트 (/release, /release-notes)
│       ├── BlogPage.tsx    # 공개 활용 아티클 (/blog)
...
| `VITE_IMIN_ADMIN_USER_IDS` | 클라이언트에서 관리자 UI 노출에 사용할 LINE userId 목록, 쉼표 구분 |
| `VITE_SHOW_SAMPLE_EVENTS` | 공개 탐색 화면에 목 행사 데이터를 병합 노출할지 여부 (`false`면 숨김) |
| `UPSTASH_REDIS_REST_URL` | Upstash Redis REST URL |
| `UPSTASH_REDIS_REST_TOKEN` | Upstash Redis REST Token |
| `IMIN_ADMIN_USER_IDS` | 서버에서 신청자 목록 조회 권한을 허용할 LINE userId 목록, 쉼표 구분 |
...
- 상위 제품 기획: `docs/product/imin-product-brief.md`
- AI 개발 아키텍처: `docs/architecture/ai-development-architecture.md`
- 사용자 관리자 벤치마킹/계획: `docs/eventus-user-admin-benchmark-2026-05-23.md`
- 작업 규칙: `AGENTS.md`
